...
func InitializeWorkspaceDatabase(db *sql.DB) error {
	...
	for _, query := range schema.AudienceIngestTableDefinitions() {
		if _, err := db.Exec(query); err != nil {
	...
	for _, query := range schema.CustomerAuthorityTableDefinitions() {
		if _, err := db.Exec(query); err != nil {
			return fmt.Errorf("failed to create customer authority compatibility table: %w", err)
		}
	}
	...
	for _, query := range schema.DeliveryTableDefinitions() {
		if _, err := db.Exec(query); err != nil {
